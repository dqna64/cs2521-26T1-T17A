...
// /**
//  * Create a new tree node with value `x` and return a pointer to it.
//  */
...
// /**
//  * Create a new tree node with value `x` and insert into the tree rooted at
//  * `root`. Return the new root of the tree.
//  */
...
struct node *bstNew(void) { return NULL; }
...
void bstPrint(struct node *t, FILE *out) { doBstPrint(t, out); }

// Prints a struct node *to stdout.
void bstShow(struct node *t) { bstPrint(t, stdout); }
...
static void doBstPrint(struct node *t, FILE *out) {
  ...
  if (proot->height >= MAX_HEIGHT) {
    fprintf(out, "(Tree is taller than %d; may be drawn incorrectly.)\n",
            MAX_HEIGHT);
  }
